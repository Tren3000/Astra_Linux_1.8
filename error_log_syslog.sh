...
echo "Запускайте скрипт от sudo-пользователя  "
...
if [ ! -w "/etc/syslog-ng/syslog-ng.conf" ]; then
    echo "Ошибка! Не установлен syslog-ng, либо, у Вас не OS Astra Linux (error_03)"
        exit 3
fi
...
if sudo cp /etc/syslog-ng/syslog-ng.conf /etc/syslog-ng/syslog-ng.conf.bak
    then 
        echo "Создаю бэкап, на всякий пожарный ... "
    else
        echo "Бэкап пошёл не по плану, хз почему, но пофиг! Продолжаем! "
fi

echo "Введите имя лога, в виде имя_лога.log :         "; read log_name

echo "Создание log-файла ... "
...
if [ ! -w "/tmp/$pipe_name" ]
    then
        echo "Ошибка! Нехватает прав для создания именованного канала (error_05)"
    exit 5
fi
echo "Файл именованного канала создан! "
echo "Запись в конфигурационный файл syslog-ng ... "
if [ ! -w "/etc/syslog-ng/syslog-ng.conf" ]
    then
        echo "Ошибка! Нехватает прав для редактирования настроек syslog-ng (error_06)"
    exit 6
fi

echo "Идет запись в файл конфигурации syslog-ng ... "
...
echo " Запись в конфигурационный файл прошла успешно! "

echo " Перезапускаю службу syslog-ng ... "

sudo systemctl restart syslog-ng || echo " Перезагрузка службы syslog-ng завершилось ошибкой, проверьте настройки системы или перезагрузите службу syslog-ng вручную ! (error_07)"; exit 7
